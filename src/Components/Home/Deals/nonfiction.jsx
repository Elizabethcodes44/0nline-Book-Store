...
export default function NonFiction () {
  ...
  useEffect(() => {
    ...
    const filteredBooks = data.Books.filter(
        (book) => book.best_seller && book.genre === "non-fiction"
    )
      ...
      {hasBooks ? (
        ...
        {deals.map((book, index) => (
                <div key={index} className="book-item border p-4 rounded-md shadow-md">
                  ...
                  <img src= {love} className="w-8 h-8 cursor-pointer mr-4"
                  onClick={handleAddToWishlist}
                  alt="Add to wishlist"/>
                  <button  className="bg-brown text-white py-2 px-4 rounded"
                  onClick={() => handleAddToCart(book)} >Add to cart
                  </button>
                </div>
                </div>
              )) }
